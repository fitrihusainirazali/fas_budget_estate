...
using MVC_SYSTEM.ModelsBudget; //fitri add
...
namespace MVC_SYSTEM.Controllers
{
    public class LoginController : Controller
    {
        ...
        private MVC_SYSTEM_ModelsBudget db2 = new MVC_SYSTEM_ModelsBudget(); //fitri add
                ...
                if (string.IsNullOrEmpty(Login.fldUserName) == false && string.IsNullOrEmpty(Login.fldUserPassword) == false)
                {
                    ...
                    if (user != null)
                    {
                        if (user.fldNegaraID == 0 || user.fldSyarikatID == 0 || user.fldWilayahID == 0)
                        {
                            ...
                            {
                                //start fitri add
                                var bgtuser = db2.bgt_Users.Where(u => u.usr_ID == Login.fldUserName.ToUpper()).SingleOrDefault();
                                if(bgtuser != null)
                                {
                                    JavaScriptSerializer js = new JavaScriptSerializer();
                                    string data = js.Serialize(user);
                                    ...
                                    //getConfig.AddUserAuditTrail(user.fldUserID, "Login to CC"); //for audit trail
