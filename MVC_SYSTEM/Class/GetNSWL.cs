...
namespace MVC_SYSTEM.Class
{
    public class GetNSWL
    {
        ...
        public void GetData(out int ? NegaraID, out int ? SyarikatID, out int?  WilayahID, out int ? LadangID, int ? userid, string username)
        {
            ...
            // fitri add
            //var bgtuserdata = db2.bgt_Users.Where(w => w.usr_ID.ToLower() == username.ToLower()).ToList();

            //if(bgtuserdata.Count() > 0)
            //{
            //    SyarikatID = bgtuserdata.FirstOrDefault().fld_SyarikatID;
            //}
